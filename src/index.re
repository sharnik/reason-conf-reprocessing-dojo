...
let bananaSplashImg =
  Draw.loadImage(~filename="./assets/splash_yellow_small.png");
...
let cutting = (mousePos, fruit) =>
  ...
  | Whole =>
    ...
    let (fX, fY) = fruit.pos;
    let {width, height} = fruit.size;
    (mouseX >= fX && mouseX <= fX + width)
    && mouseY >= fY
    && mouseY <= fY
    + height ?
      Slice : Whole;
...
let drawFruit = (env, fruit) => {
  ...
  let f = img =>
    Draw.image(img, ~pos=fruit.pos, ~width=100, ~height=100, env);
  switch (fruit.state) {
...
let draw = (state, env) => {
  ...
  {fruits: Array.append(updatedFruits, addFruit(env))};
};
